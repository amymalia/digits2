...
Meteor.methods({
  checkWeather(latitude, longitude) {
    ...
    try {
      ...
      //console.log(devices);
      const weather = { latitude, longitude, description, temperature, windSpeed, clouds, name, radiation, radiationForecast };
      if (Weather.find().count() === 0) {
        ...
        Weather.update(weather._id, {
          $set: { latitude, longitude, description, temperature, windSpeed, clouds, name, radiation, radiationForecast },
        });
      }
      return weather;
    } catch (e) {
